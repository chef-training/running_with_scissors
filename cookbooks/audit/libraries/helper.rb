...
module ReportHelpers
  ...
  # taking node['audit'] as parameter so that it can be called from the
  # chef-server fetcher as well audit['collector'] is the legacy reporter,
  #
  # A single reporter or an array of reporters are supported.
  # @returns an Array of reporters.
  def get_reporters(audit)
    if audit['collector']
      ...
      return Array(audit['collector'])
    end
    Array(audit['reporter'])
  end
end
